...
Output
------
Each generated file (text or ``.npy``) contains four columns ordered as::
...
LOG10_Q_MIN = math.log10(1.0e-7) # math.log10(2.6e-5)   # lower limit from Table 6 discussion (~2.6e-5)
LOG10_Q_MAX = math.log10(2.6e-5) # math.log10(3.0e-2)   # upper limit used in MOA sample (0.03)
LOG10_S_MIN = math.log10(0.1)      # 0.1 <= s <= 10 range from survey sensitivity
LOG10_S_MAX = math.log10(10.0)

rundes = 'data_challenge_lowq'
sources_file = './gulls_surot2d_H2023.sources'
file_ext = ''
nl = 500      # draws per file
nf = 5          # files per field
overwrite_existing = True

delineator = " "
header = False
_HEADER_LINE = 'q, s, inc (deg), p (deg)'
...
def worker(task: tuple[int, int]) -> None:
    ...
    combined_array[:, 0] = q_array
    combined_array[:, 1] = s_array
